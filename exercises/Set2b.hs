...
countdownHelper :: String -> Integer -> String
countdownHelper s 0 = s
countdownHelper s n = countdownHelper (s ++ show n ++ "... ") (n - 1)
...
smallestDivisor n = head (smallestDivisorHelper n (sieve [2..n]))
smallestDivisorHelper :: Integer -> [Integer] -> [Integer]
smallestDivisorHelper n l
    | mod n (head l) > 0 = smallestDivisorHelper n (drop 1 l)
    | otherwise = l
sieve :: [Integer] -> [Integer]
sieve l = check (head l) (drop 1 l)
check :: Integer -> [Integer] -> [Integer]
check x l
    | null l = [x]
    | rem (head l) x > 0 = x : check (head l) (drop 1 l)
    | otherwise = check x (drop 1 l)
...
biggestPrimeAtMost n = biggestPrimeAtMostHelper (reverse (sieve [2..n]))
biggestPrimeAtMostHelper :: [Integer] -> Integer
biggestPrimeAtMostHelper l
    | isPrime (head l) = head l
    | otherwise = biggestPrimeAtMostHelper (drop 1 l)
